...
void DW_RESET_Pin_CallBack(void* args)
{
    signalResetDone = 1;
}
...
void setup_DW1000RSTnIRQ(int enable)
{
    ...
    if (enable) {
        rt_pin_mode(DW_RESET_Pin, PIN_MODE_INPUT);
        rt_pin_attach_irq(DW_RESET_Pin, PIN_IRQ_MODE_RISING, DW_RESET_Pin_CallBack, RT_NULL);
        rt_pin_irq_enable(DW_RESET_Pin, PIN_IRQ_ENABLE);
    } else {
        rt_pin_irq_enable(DW_RESET_Pin, PIN_IRQ_DISABLE);
        rt_pin_detach_irq(DW_RESET_Pin);
        rt_pin_mode(DW_RESET_Pin, PIN_MODE_OUTPUT_OD);
    }
}
